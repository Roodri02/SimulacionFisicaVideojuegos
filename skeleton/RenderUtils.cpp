...
{
	StartCounter();
	sCamera = new Camera(PxVec3(50.0f, 55.0f, 120.0f), PxVec3(-0.6f,-0.2f,-0.7f));
...
PxShape* CreateShape(const PxGeometry& geo, const PxMaterial* pM)
{

	if (pM == nullptr)
		pM = gMaterial; // Default material

	PxShape* shape = gPhysics->createShape(geo, *pM);
	return shape;
}