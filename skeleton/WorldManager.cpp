...
WorldManager::WorldManager(PxScene* gScene_, PxPhysics* gPhysics_, PartycleSystem* particleSystem) {
	...
	PxShape* shape = CreateShape(PxBoxGeometry(150, 0.3, 200));
	plano->attachShape(*shape);
	plano->setName("Static");
	item = new RenderItem(shape, plano, { 1.0, 1.0, 1.0, 1 });
	gScene->addActor(*plano);

	PxRigidStatic* planoPared1 = gPhysics->createRigidStatic(PxTransform({ 50 , 130, -230 }));
	PxShape* shape1 = CreateShape(PxBoxGeometry(200, 200, 2));
	planoPared1->attachShape(*shape1);
	planoPared1->setName("Static");
	item = new RenderItem(shape1, planoPared1, { 1.0, 1.0, 1.0, 1 });
	gScene->addActor(*planoPared1);

	PxRigidStatic* planoPared2 = gPhysics->createRigidStatic(PxTransform({ 180 , 130, -30 }));
	PxShape* shape2 = CreateShape(PxBoxGeometry(2, 200, 200));
	planoPared2->attachShape(*shape2);
	planoPared2->setName("Static");
	item = new RenderItem(shape2, planoPared2, { 1.0, 1.0, 1.0, 1 });
	gScene->addActor(*planoPared2);


	PxRigidStatic* planoPared3 = gPhysics->createRigidStatic(PxTransform({ -80 , 130, -30 }));
	PxShape* shape3 = CreateShape(PxBoxGeometry(2, 200, 200));
	planoPared3->attachShape(*shape3);
	planoPared3->setName("Static");
	item = new RenderItem(shape3, planoPared3, { 1.0, 1.0, 1.0, 1 });
	gScene->addActor(*planoPared3);


	PxRigidStatic* techo = gPhysics->createRigidStatic(PxTransform({ 50,320,-30 }));
	PxShape* shape4 = CreateShape(PxBoxGeometry(150, 0.3, 200));
	techo->attachShape(*shape4);
	techo->setName("Static");
	item = new RenderItem(shape4, techo, { 1.0, 1.0, 1.0, 1 });
	...
	enemigosTipo2 = 30;
	enemigosTipo3 = 40;
	enemigosEliminarRonda1 = 10;
	enemigosEliminarRonda2 = 15;
	enemigosEliminarRonda3 = 20;
...
void WorldManager::update(double t)
{
	...
	if (enemigosActuales <= 0 && enemigosEliminados > enemigosEliminar) {
...
void WorldManager::comienzaRonda1() {
	...
	enemigosEliminar = enemigosEliminarRonda1;
	particleSystem_->desactivaFirework();
	PxMaterial* gMaterial = gPhysics->createMaterial(0.5f, 0.5f, 0.6f);
	RigidGaussianParticleGenerator* rgpg = new RigidGaussianParticleGenerator(CreateShape(PxBoxGeometry(6, 6, 6), gMaterial), 1, 0.9, 1, 5, { 45,0.1,45 },{0.01,0.01,0.01});
...
void WorldManager::comienzaRonda2() {
	...
	enemigosEliminar = enemigosEliminarRonda2;
	particleSystem_->desactivaFirework();
	PxMaterial* gMaterial = gPhysics->createMaterial(0.5f, 0.5f, 0.6f);
	RigidGaussianParticleGenerator* rgpg = new RigidGaussianParticleGenerator(CreateShape(PxBoxGeometry(6, 6, 6), gMaterial), 1, 0.9, 1, 5, { 45,0.1,45 },{20,0.01,4});
	rgpg->setMeanPos({ 50,50,0 }, { 0,0,0 });
	rigidParticleGen.push_back(rgpg);
}
